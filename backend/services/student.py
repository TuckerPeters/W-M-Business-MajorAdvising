...
class StudentService:
    ...
    def parse_term(self, term: str) -> Tuple[str, int]:
        ...
        # Handle 6-digit term codes (e.g. 202620)
        if term.isdigit() and len(term) == 6:
            code_map = {"10": "Spring", "20": "Summer", "30": "Fall"}
            season = code_map.get(term[4:])
            if not season:
                raise ValueError(f"Invalid term code: {term}. Last two digits must be 10, 20, or 30")
            return season, int(term[:4])

        parts = term.split()
        if len(parts) != 2:
            raise ValueError(f"Invalid term format: {term}. Expected 'Season Year' (e.g., 'Fall 2025') or a 6-digit code (e.g., '202530')")
    ...
    def get_milestones(self, user_id: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        milestones.sort(key=lambda m: m.get("order", 999))
        return milestones
